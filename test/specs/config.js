...
describe('Config files', () => {
    beforeEach(function() {
        const testCase = mockFsHelper.duplicateFSInMemory(
            path.join(__dirname, '../resources/testcase')
        );
        mock(testCase);
    });
    ...
    it('should respect editorConfig (space) when provided', () => {
        const config = configHelpers.getConfig(
            'with-editorconfig-space/controller.js'
        );
    ...
    it('should respect editorConfig (tab) when provided', () => {
        const config = configHelpers.getConfig(
            'with-editorconfig-tab/controller.js'
        );
        expect(config.style.indentation).to.equal('\t');
    ...
    it('should merge configs properly with rc file', () => {
        ...
        expect(config.style.indentation).to.equal('  ');
        expect(config.specialImportsMap).to.have.property('specialAction');
    });
    ...
    it('should merge configs properly with a flat specialImports', () => {
        const config = configHelpers.getConfig('without-rc/controller.js', {
            specialImports: { map: 'commons/map' }
        });
        expect(config.specialImportsMap).to.have.property('map');
    });
